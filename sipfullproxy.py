...
class UDPHandler(SocketServer.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        dateLocal = time.strftime("%d %b %Y %H:%M:%S ", time.localtime())
        file = open("xluleymSipLogger.txt", "a")  # append mode
        file.write("%s : %s sa zaregistroval\n" % (dateLocal, fromm.split("@")[0]))
        file.close()
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            if registrar.has_key(destination) and self.checkValidity(destination):
                ...
                dateLocal = time.strftime("%d %b %Y %H:%M:%S ", time.localtime())
                file = open("xluleymSipLogger.txt", "a")
                file.write("%s : %s vyzvana %s \n" % (dateLocal, origin.split("@")[0], destination.split("@")[0]))
                file.close()
            else:
    ...
    def processAck(self):
        ...
        if len(destination) > 0:
            ...
            if registrar.has_key(destination):
                ...
                dateLocal = time.strftime("%d %b %Y %H:%M:%S ", time.localtime())
                file = open("xluleymSipLogger.txt", "a")
                file.write("%s : %s zdvihol hovor\n" % (dateLocal, destination.split("@")[0]))
                file.close()
        ...
        if len(destination) > 0:
            if registrar.has_key(destination) and self.checkValidity(destination):
                ...
                if rx_bye.search(self.data[0]):
                    dateLocal = time.strftime("%d %b %Y %H:%M:%S ", time.localtime())
                    file = open("xluleymSipLogger.txt", "a")
                    file.write("%s : %s zlozil hovor\n-------\n" % (dateLocal, origin.split("@")[0]))
                    file.close()
